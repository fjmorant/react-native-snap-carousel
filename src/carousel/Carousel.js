...
export default class Carousel extends Component {
  ...
  constructor(props) {
    ...
    this._onScrollBeginDrag = this._snapEnabled
      ? this._onScrollBeginDrag.bind(this)
      : null;
    this._onScrollEnd =
      this._snapEnabled || props.autoplay ? this._onScrollEnd.bind(this) : null;
    ...
    if (props.onScrollViewScroll) {
      console.warn(
        "react-native-snap-carousel: Prop `onScrollViewScroll` is deprecated. Please use `onScroll` instead"
      );
    }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const nextActiveItem =
      activeItem || activeItem === 0 ? activeItem : nextFirstItem;
    ...
    if (
      (childrenLength && interpolators.length !== childrenLength) ||
      hasNewSliderWidth ||
      hasNewSliderHeight ||
      hasNewItemWidth ||
      hasNewItemHeight
      ...
      nextFirstItem !== previousFirstItem &&
      nextFirstItem !== activeItem
    ) {
      this.setState({
  ...
  _getCustomIndex(index, props = this.props) {
    ...
    return IS_RTL && !props.vertical ? childrenLength - index - 1 : index;
  }
  ...
  _getCenter(offset) {
    ...
    return (
      offset - containerMargin + (vertical ? sliderHeight : sliderWidth) / 2
    );
  }
  ...
  _getSlideAnimation(index, toValue) {
    ...
    return Animated.parallel([
      Animated["timing"](this.state.interpolators[index].opacity, {
        ...animationCommonOptions,
        easing: Easing.linear
      }),
      Animated[animationFunc](this.state.interpolators[index].scale, {
        ...animationCommonOptions
      })
    ]);
  }
  ...
  _onScroll(event) {
    ...
    if (
      ...
      !this._isShortSnapping &&
      (this._scrollStartActive !== newActiveItem ||
        !this._hasFiredEdgeItemCallback) &&
  ...
  snapToItem(index, animated = true, fireCallback = true, initial = false) {
    ...
    if (itemsLength > 0 && index >= itemsLength) {
      index = itemsLength - 1;
      if (
        this._scrollStartActive === itemsLength - 1 &&
        this._hasFiredEdgeItemCallback
      ) {
        fireCallback = false;
      }
    ...
    if (this._scrollview) {
      ...
      if (enableMomentum) {
        ...
        this._scrollToTimeout = setTimeout(() => {
          if (
            scrollPosition === this._currentContentOffset &&
            this._canFireCallback
          ) {
            this._onSnap(index);
          }
  ...
  render() {
    ...
    const horizontalMargin =
      carouselHorizontalPadding || carouselHorizontalPadding === 0
        ? carouselHorizontalPadding
        : (sliderWidth - itemWidth) / 2;
    const verticalMargin =
      carouselVerticalPadding || carouselVerticalPadding === 0
        ? carouselVerticalPadding
        : (sliderHeight - itemHeight) / 2;
    const style = [
      containerCustomStyle || {},
